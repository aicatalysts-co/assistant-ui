...
describe("Interactables registration", () => {
  ...
  it("only restores detached tool-created state in the same thread", async () => {
    root = mount({ threadId: "thread-a", threadMessages: [createCall("shared")] });
    ...
    clientHolder.client = makeClient([createCall("shared")], undefined, "thread-b");
  ...
  it("installs the update tool UI once per name and removes it with the last anchor", () => {
    ...
    const second = root.getValue().register(reg("n2", { updateRender: render }));
